package com.dylanc.retrofit.compiler

import com.dylanc.retrofit.annotations.BaseUrl
import com.dylanc.retrofit.annotations.DebugUrl
...
class UrlProcessor : AbstractProcessor() {
  ...
  override fun process(
    ...
    val hashMapType = ParameterizedTypeName.get(
    ...
    val typeSpec =
      TypeSpec.classBuilder(ClassName.get("com.dylanc.retrofit", "UrlConfig"))
    ...
    if (hasBinding) {
      try {
        JavaFile.builder("com.dylanc.retrofit", typeSpec)
          .build()
          .writeTo(filer)
      } catch (e: IOException) {
        e.printStackTrace()
